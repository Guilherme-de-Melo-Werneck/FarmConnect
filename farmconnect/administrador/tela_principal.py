...
class TelaAdminDashboard:
    ...
    def load_pacientes(self, e=None, paciente=None):
        ...
        # Exemplo mock (vai pro banco depois)
        pacientes_mock = [
                    ...
                    cells=[
                        ...
                        ft.DataCell(
                            ft.Container(
                                content=ft.Text(
                                    p[5],
                                    color="#D97706" if p[5] == "Pendente" else "#15803D" if p[5] == "Aprovado" else "#DC2626",
                                    weight="bold"
                                ),
                                padding=ft.padding.symmetric(horizontal=6, vertical=2),
                                bgcolor="#FEF3C7" if p[5] == "Pendente" else "#D1FAE5" if p[5] == "Aprovado" else "#FEE2E2",
                                border_radius=6,
                            )
                        ),
                        ft.DataCell(
                            ft.Row([
                                ft.IconButton(
                                    ...
                                    icon_color="#059669",
                                    tooltip="Aprovar",
                                    on_click=lambda e: print("Aprovar paciente", p[0])
                                ),
                                ft.IconButton(
                                    icon=ft.icons.CANCEL_OUTLINED,
                                    icon_color="#DC2626",
                                    tooltip="Recusar",
                                    on_click=lambda e: print("Recusar paciente", p[0])
                                ),
                            ], spacing=4)
                        )
                    ]
        ...
        self.campo_nome_p = ft.TextField(label="Nome", value=paciente.get("nome") if paciente else "")
        self.campo_cpf_p = ft.TextField(label="CPF", value=paciente.get("cpf") if paciente else "")
        self.campo_email_p = ft.TextField(label="Email", value=paciente.get("email") if paciente else "")
        self.campo_telefone_p = ft.TextField(label="Telefone", value=paciente.get("telefone") if paciente else "")
        ...
        painel_detalhes = ft.Container(
            ...
            content=ft.Column([
                ft.Text("Detalhes do Paciente", size=20, weight="bold", color="#059669"),
                ft.Divider(),
                self.campo_nome_p,
                self.campo_cpf_p,
                self.campo_email_p,
                self.campo_telefone_p,
                ft.Container(height=20),
                ft.Row([
                    ft.ElevatedButton(
                        "Salvar",
                        ...
                        expand=True,
                        style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=12)),
                        on_click=lambda e: self.load_pacientes()  # simulação
                    ),
                    ft.OutlinedButton(
                        "Cancelar",
                        expand=True,
        ...
        self.current_view.controls.append(
            ft.Container(
                padding=25,
                content=ft.Column([
                    ft.Text("Lista de Pacientes", size=24, weight="bold", color="#065F46"),
                    ft.Container(height=20),
                    ft.Container(
                        padding=16,
                        bgcolor="white",
                        border_radius=12,
                        shadow=ft.BoxShadow(blur_radius=8, color="#CBD5E1"),
                        content=tabela_pacientes
                    ),
                    ft.Container(height=20),
                    ft.Row([
                        ft.Container(
                            expand=2 if not self.editando_paciente else 1,
                            content=ft.Container()
                        ),
                        ft.Container(width=20),
                        painel_detalhes
